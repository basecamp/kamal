...
class CliAppTest < CliTestCase
  ...
  test "exec" do
    run_command("exec", "ruby -v").tap do |output|
      ...
      assert_match /docker run --rm --name app-web-exec-latest-[0-9a-f]{6} --network kamal --env-file \.kamal\/apps\/app\/env\/roles\/web\.env --log-opt max-size="10m" dhh\/app:latest ruby -v/, output
    end
  end
  ...
  test "exec detach" do
    run_command("exec", "--detach", "ruby -v").tap do |output|
      assert_match /docker run --detach --name app-web-exec-latest-[0-9a-f]{6} --network kamal --env-file \.kamal\/apps\/app\/env\/roles\/web\.env --log-opt max-size="10m" dhh\/app:latest ruby -v/, output
    end
  end
  ...
  test "exec interactive" do
    ...
    SSHKit::Backend::Abstract.any_instance.expects(:exec)
      .with(regexp_matches(/ssh -t root@1\.1\.1\.1 -p 22 'docker run -it --rm --name app-web-exec-latest-[0-9a-f]{6} --network kamal --env-file \.kamal\/apps\/app\/env\/roles\/web\.env --log-opt max-size="10m" dhh\/app:latest ruby -v'/))
