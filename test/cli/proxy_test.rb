...
class CliProxyTest < CliTestCase
  test "boot" do
    run_command("boot").tap do |output|
      ...
      assert_match "docker run --name kamal-proxy --network kamal --detach --restart unless-stopped --volume kamal-proxy-config:/home/kamal-proxy/.config/kamal-proxy $(cat .kamal/proxy/options || echo \"--publish 80:80 --publish 443:443 --log-opt max-size=10m #{Kamal::Configuration::Proxy::DEFAULT_IMAGE}\")", output
    end
  end
  ...
  test "boot correct version" do
    ...
    run_command("boot").tap do |output|
      assert_match "docker login", output
      assert_match "docker container start kamal-proxy || docker run --name kamal-proxy --network kamal --detach --restart unless-stopped --volume kamal-proxy-config:/home/kamal-proxy/.config/kamal-proxy $(cat .kamal/proxy/options || echo \"--publish 80:80 --publish 443:443 --log-opt max-size=10m #{Kamal::Configuration::Proxy::DEFAULT_IMAGE}\")", output
    end
  ensure
    Thread.report_on_exception = false
  end
  ...
  test "reboot" do
    ...
    run_command("reboot", "-y").tap do |output|
      ...
      assert_match "docker run --name kamal-proxy --network kamal --detach --restart unless-stopped --volume kamal-proxy-config:/home/kamal-proxy/.config/kamal-proxy $(cat .kamal/proxy/options || echo \"--publish 80:80 --publish 443:443 --log-opt max-size=10m #{Kamal::Configuration::Proxy::DEFAULT_IMAGE}\") on 1.1.1.1", output
      ...
      assert_match "docker run --name kamal-proxy --network kamal --detach --restart unless-stopped --volume kamal-proxy-config:/home/kamal-proxy/.config/kamal-proxy $(cat .kamal/proxy/options || echo \"--publish 80:80 --publish 443:443 --log-opt max-size=10m #{Kamal::Configuration::Proxy::DEFAULT_IMAGE}\") on 1.1.1.2", output
  ...
  test "boot_config set" do
    run_command("boot_config", "set").tap do |output|
      %w[ 1.1.1.1 1.1.1.2 ].each do |host|
        ...
        assert_match "Uploading \"--publish 80:80 --publish 443:443 --log-opt max-size=10m #{Kamal::Configuration::Proxy::DEFAULT_IMAGE}\" to .kamal/proxy/options on #{host}", output
      end
    end
  end
  ...
  test "boot_config set no publish" do
    run_command("boot_config", "set", "--publish", "false").tap do |output|
      %w[ 1.1.1.1 1.1.1.2 ].each do |host|
        assert_match "Running /usr/bin/env mkdir -p .kamal/proxy on #{host}", output
        assert_match "Uploading \"--log-opt max-size=10m #{Kamal::Configuration::Proxy::DEFAULT_IMAGE}\" to .kamal/proxy/options on #{host}", output
      end
    end
  end
  ...
  test "boot_config set custom max_size" do
    run_command("boot_config", "set", "--log-max-size", "100m").tap do |output|
      %w[ 1.1.1.1 1.1.1.2 ].each do |host|
        assert_match "Running /usr/bin/env mkdir -p .kamal/proxy on #{host}", output
        assert_match "Uploading \"--publish 80:80 --publish 443:443 --log-opt max-size=100m #{Kamal::Configuration::Proxy::DEFAULT_IMAGE}\" to .kamal/proxy/options on #{host}", output
      end
    end
  end
  ...
  test "boot_config set no log max size" do
    run_command("boot_config", "set", "--log-max-size=").tap do |output|
      %w[ 1.1.1.1 1.1.1.2 ].each do |host|
        assert_match "Running /usr/bin/env mkdir -p .kamal/proxy on #{host}", output
        assert_match "Uploading \"--publish 80:80 --publish 443:443 #{Kamal::Configuration::Proxy::DEFAULT_IMAGE}\" to .kamal/proxy/options on #{host}", output
      end
    end
  end
  ...
  test "boot_config set custom ports" do
    run_command("boot_config", "set", "--http-port", "8080", "--https-port", "8443").tap do |output|
      %w[ 1.1.1.1 1.1.1.2 ].each do |host|
        assert_match "Running /usr/bin/env mkdir -p .kamal/proxy on #{host}", output
        assert_match "Uploading \"--publish 8080:80 --publish 8443:443 --log-opt max-size=10m #{Kamal::Configuration::Proxy::DEFAULT_IMAGE}\" to .kamal/proxy/options on #{host}", output
      end
    end
  end
  ...
  test "boot_config set bind IP" do
    run_command("boot_config", "set", "--publish-host-ip", "127.0.0.1").tap do |output|
      %w[ 1.1.1.1 1.1.1.2 ].each do |host|
        assert_match "Running /usr/bin/env mkdir -p .kamal/proxy on #{host}", output
        assert_match "Uploading \"--publish 127.0.0.1:80:80 --publish 127.0.0.1:443:443 --log-opt max-size=10m #{Kamal::Configuration::Proxy::DEFAULT_IMAGE}\" to .kamal/proxy/options on #{host}", output
      end
    end
  end
  ...
  test "boot_config set multiple bind IPs" do
    run_command("boot_config", "set", "--publish-host-ip", "127.0.0.1", "--publish-host-ip", "::1").tap do |output|
      %w[ 1.1.1.1 1.1.1.2 ].each do |host|
        assert_match "Running /usr/bin/env mkdir -p .kamal/proxy on #{host}", output
        assert_match "Uploading \"--publish 127.0.0.1:80:80 --publish 127.0.0.1:443:443 --publish [::1]:80:80 --publish [::1]:443:443 --log-opt max-size=10m #{Kamal::Configuration::Proxy::DEFAULT_IMAGE}\" to .kamal/proxy/options on #{host}", output
      end
    end
  end
  ...
  test "boot_config set docker options" do
    run_command("boot_config", "set", "--docker_options", "label=foo=bar", "add_host=thishost:thathost").tap do |output|
      %w[ 1.1.1.1 1.1.1.2 ].each do |host|
        assert_match "Running /usr/bin/env mkdir -p .kamal/proxy on #{host}", output
        assert_match "Uploading \"--publish 80:80 --publish 443:443 --log-opt max-size=10m --label=foo=bar --add_host=thishost:thathost #{Kamal::Configuration::Proxy::DEFAULT_IMAGE}\" to .kamal/proxy/options on #{host}", output
      end
    end
  end

  test "boot_config set image" do
    run_command("boot_config", "set", "--image", "registry.example.com/namespace/kamal-proxy").tap do |output|
      %w[ 1.1.1.1 1.1.1.2 ].each do |host|
        assert_match "Running /usr/bin/env mkdir -p .kamal/proxy on #{host}", output
        assert_match "Uploading \"--publish 80:80 --publish 443:443 --log-opt max-size=10m registry.example.com/namespace/kamal-proxy\" to .kamal/proxy/options on #{host}", output
      end
    end
  end
  ...
  test "boot_config get" do
    SSHKit::Backend::Abstract.any_instance.expects(:capture_with_info)
      .with(:cat, ".kamal/proxy/options", "||", :echo, "\"--publish 80:80 --publish 443:443 --log-opt max-size=10m #{Kamal::Configuration::Proxy::DEFAULT_IMAGE}\"")
