...
class CommandsProxyTest < ActiveSupport::TestCase
  ...
  test "run" do
    assert_equal \
      "docker run --name kamal-proxy --network kamal --detach --restart unless-stopped --volume kamal-proxy-config:/home/kamal-proxy/.config/kamal-proxy $(cat .kamal/proxy/options || echo \"--publish 80:80 --publish 443:443 --log-opt max-size=10m #{Kamal::Configuration::Proxy::DEFAULT_IMAGE}\")",
  ...
  test "get_boot_options" do
    assert_equal \
      "cat .kamal/proxy/options || echo \"--publish 80:80 --publish 443:443 --log-opt max-size=10m #{Kamal::Configuration::Proxy::DEFAULT_IMAGE}\"",
