...
class CommandsAppTest < ActiveSupport::TestCase
  ...
  test "stop" do
    assert_equal \
      "docker ps --quiet --filter label=service=app --filter label=role=web --latest | xargs docker stop -t 10",
  ...
  test "info" do
    assert_equal \
      "docker ps --filter label=service=app --filter label=role=web --latest",
  ...
  test "info with destination" do
    @destination = "staging"
    assert_equal \
      "docker ps --filter label=service=app --filter label=destination=staging --filter label=role=web --latest",
      new_command.info.join(" ")
  end
  ...
  test "logs" do
    assert_equal \
      "docker ps --quiet --filter label=service=app --filter label=role=web --latest | xargs docker logs 2>&1",
      new_command.logs.join(" ")

    assert_equal \
      "docker ps --quiet --filter label=service=app --filter label=role=web --latest | xargs docker logs --since 5m 2>&1",
      new_command.logs(since: "5m").join(" ")

    assert_equal \
      "docker ps --quiet --filter label=service=app --filter label=role=web --latest | xargs docker logs --tail 100 2>&1",
      new_command.logs(lines: "100").join(" ")

    assert_equal \
      "docker ps --quiet --filter label=service=app --filter label=role=web --latest | xargs docker logs --since 5m --tail 100 2>&1",
      new_command.logs(since: "5m", lines: "100").join(" ")

    assert_equal \
      "docker ps --quiet --filter label=service=app --filter label=role=web --latest | xargs docker logs 2>&1 | grep 'my-id'",
      new_command.logs(grep: "my-id").join(" ")

    assert_equal \
      "docker ps --quiet --filter label=service=app --filter label=role=web --latest | xargs docker logs --since 5m 2>&1 | grep 'my-id'",
      new_command.logs(since: "5m", grep: "my-id").join(" ")
  end

  test "follow logs" do
    assert_match \
      "docker ps --quiet --filter label=service=app --filter label=role=web --latest | xargs docker logs --timestamps --tail 10 --follow 2>&1",
      new_command.follow_logs(host: "app-1")

    assert_match \
      "docker ps --quiet --filter label=service=app --filter label=role=web --latest | xargs docker logs --timestamps --tail 10 --follow 2>&1 | grep \"Completed\"",
  ...
  test "current_container_id" do
    assert_equal \
      "docker ps --quiet --filter label=service=app --filter label=role=web --latest",
  ...
  test "current_container_id with destination" do
    @destination = "staging"
    assert_equal \
      "docker ps --quiet --filter label=service=app --filter label=destination=staging --filter label=role=web --latest",
      new_command.current_container_id.join(" ")
  end
  ...
  test "current_running_version" do
    assert_equal \
      "docker ps --filter label=service=app --filter label=role=web --format \"{{.Names}}\" --latest | sed 's/-/\\n/g' | tail -n 1",
