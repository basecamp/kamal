...
In recent years, there has been a surge in commercial solutions that simplify the deployment of web applications. Heroku was among the first to offer an impressive service that remained ahead of the competition for a long time. Other great options such as Fly.io and Render are now available. Additionally, hosted Kubernetes has made things more accessible on several platforms like AWS, GCP, and Digital Ocean. However, these are all cloud-based options that require renting computers at a premium. If you plan to run on your hardware or desire a clear migration path to do so in the future, it's crucial to evaluate how tied you are to these commercial platforms to avoid getting caught off guard by high bills.

MRSK aims to provide the convenience of deploying web applications anywhere, just like commercial offerings. It doesn't matter whether you prefer low-cost cloud options like Digital Ocean, Hetzner, OVH, or bare metal colocation. MRSK can handle it all. All you need to do is feed the config file with IP addresses of vanilla Ubuntu servers, which require no preparation beyond adding an SSH key, and you'll be up and running in no time.

This approach delivers excellent portability, allowing you to deploy your web app on several clouds with ease or purchase your hardware and deploy it to the cloud for more capacity during a seasonal surge. By avoiding being tied to a single provider, you have access to many attractive options.

Ultimately, MRSK streamlines the process of going into production using open-source tools that are not tied to any commercial platform. However, it's not a zero-complexity solution. If you find basic Linux or Docker challenging, a fully managed service might be a better option. However, as soon as you're comfortable with those concepts, you can use MRSK to deploy your web application.
